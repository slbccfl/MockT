class RolesController < ApplicationController
	def new
		...
		if @role.side == "p"
			...
			@team_members = TeamMember.joins(:team_assignments).where(team_assignments: {team_id: @round.p_team_id})
		else
			@team = Team.find_by(id: @round.d_team_id)
			@team_members = TeamMember.joins(:team_assignments).where(team_assignments: {team_id: @round.d_team_id})
		end
	end
	def create
